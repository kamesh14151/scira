...
export async function POST(req: NextRequest) {
  try {
    ...
    // Professional header with AJ branding and chat title
    ...
    // Draw the AJ logo (PNG image) at the specified position and width
    ...
    function drawSciraLogo(x: number, yTop: number, width: number, color = rgb(0, 0, 0)) {
      // Get the logo dimensions to maintain aspect ratio
      const logoDims = logoImage.scale(1);
      const aspectRatio = logoDims.height / logoDims.width;
      const height = width * aspectRatio;
      
      // Convert yTop (top position) to y (bottom position) for PDF coordinate system
      const y = yTop - height;
