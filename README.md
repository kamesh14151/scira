...
4. **Set the search engine name**:
   - Enter `AJ` in the "Search engine" field.
...
After completing these steps, you should be able to use AJ as your default search engine in Chrome.
...
2. Build the Docker image:
   ```bash
   docker build -t aj.app .
   ```
3. Run the container:
   ```bash
   docker run --env-file .env -p 3000:3000 aj.app
   ```
